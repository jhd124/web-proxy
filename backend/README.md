...
- `LICENSE_PUBLIC_KEY`：License Ed25519 公钥（原始 32 字节 base64url），用于验证发码服务签名；未配置时应用保持试用版，激活接口会拒绝 License。
- `PROXY_BUNDLED_PRO`：设为 `1`/`true`/`yes` 时跳过 License 激活，直接以 Pro 套餐运行（仅供内部或免激活打包使用）。
