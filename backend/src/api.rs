...
pub async fn run_dashboard(bind: SocketAddr, state: Arc<AppState>) -> anyhow::Result<()> {
    ...
    let app = Router::new()
        ...
        .route("/api/hosts", get(crate::hosts::list_hosts))
        .route("/api/hosts", put(crate::hosts::update_hosts))
        .route("/api/hosts/apply", post(crate::hosts::apply_hosts))
        .route("/api/hosts/revert", post(crate::hosts::revert_hosts))
