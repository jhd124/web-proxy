...
- `portal.tsx`：模块组装入口，连接请求编写器 Hook 与纯 UI，并接收 dashboard 注入的历史请求保存/创建 override 动作。
...
- 历史列表只展示通过请求编写器发送过的请求；选择历史记录后可查看响应摘要并复用到表单，`Reuse` 会回填 URL、method、search params、headers 与 body；历史项右键菜单可保存请求或用该响应创建 override。
