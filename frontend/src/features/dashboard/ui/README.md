...
- `DashboardUI*`：工作台主布局（负责把 traffic/override 触发的断点“预填新建表单”动作透传到 breakpoints 面板，把 request-composer 历史右键动作透传到 saved/override 共享状态，并按 active tab 渲染 traffic/override/breakpoints/saved/request-composer/hosts/settings）。
