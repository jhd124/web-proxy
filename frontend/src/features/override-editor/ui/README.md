...
- `OverrideRequestFormUI*`：请求匹配表单（移除顶部 Override id 区块与“Enable this override rule”开关行，`Method` 与 `Protocol` 使用 shadcn `Select` 下拉选择或 `ANY`，其中新建 override 的默认 `Method` 为 `GET`）。各字段的说明文字不再内联展示，而是收进 label 后的 `<CircleQuestionMark />` 图标 tooltip 中；`Request headers to match` 与 `Search parameters to match` 的 value 支持 `*` / `?` 通配符。
