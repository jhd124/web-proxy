...
- `HostGroupList.tsx`：泛型展示组件。接收已分组数据 `groups`（`{ host, items }[]`），通过 `renderItem` 渲染每个条目，`getItemKey` 提供 key；可选 `isGroupActive` 控制分组箭头是否高亮（绿色），`toggleLabel` 提供折叠按钮无障碍文案，`idPrefix` 用于生成稳定 DOM id。
